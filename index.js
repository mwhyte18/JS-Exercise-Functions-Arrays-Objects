...
function getCarInfoByIndex(inventory, index) {
  const inventoryIndex = inventory[index];
...
function getLastCarInfo(inventory) {
  const invItem = inventory[inventory.length -1];
  return `This is a ${invItem.car_make} ${invItem.car_model}`
}
...
function getCarInfoById(inventory, carID) {
  const carIndex = inventory[carID -1];
...
function getModelYears(inventory) {
  const yearInventory = inventory.map(function(item) { return item["car_year"];});
...
function carMaker(odometer) {
  const odomObject = {
    ...
    drive: function(distance){
      odomObject.odometer += distance;
      return odomObject.odometer;
    }
  }
  return odomObject;
}
